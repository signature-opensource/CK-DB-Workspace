...
(
    ...
    @TargetWorkspaceId int -- not null
)
as
begin
    if @TargetWorkspaceId <= 0 throw 50000, 'Workspace.InvalidWorkspaceId', 1;

    -- Only if the workspace exists, try to unplug it...
    if exists (select 1 from CK.tWorkspace where WorkspaceId = @TargetWorkspaceId)
    begin
        ...
        select @AdminGroupId = w.AdminGroupId
            ...
            where w.WorkspaceId = @TargetWorkspaceId and a.GrantLevel = 127;
        ...
        set @Cursor = cursor static read_only for
            select UserId
                from CK.tUser u
                where u.PreferredWorkspaceId = @TargetWorkspaceId;
        ...
        delete from CK.tWorkspace where WorkspaceId = @TargetWorkspaceId;
